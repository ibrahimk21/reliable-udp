...
Reads:
    summary.csv
      columns: size_bytes, drop_pct, rudp_mbps_median, rudp_fec_mbps_median,
              rudp_fec_v2_mbps_median, tcp_mbps_median
...
def load_summary():
    ...
    with open(SUMMARY_CSV) as f:
        for row in csv.DictReader(f):
            ...
            row["rudp_fec_v2_mbps_median"] = float(row.get("rudp_fec_v2_mbps_median", 0))
...
def plot_throughput_vs_loss(rows):
    ...
    for idx, size_bytes in enumerate(sizes):
        ...
        rudp_fec_v2 = [r["rudp_fec_v2_mbps_median"] for r in sub]
        ...
        ax.plot(drop_pcts[:len(sub)], rudp_fec_v2, "D-", label="RUDP-FECv2", linewidth=2)
    ...
    fig.suptitle("RUDP vs RUDP-FEC vs RUDP-FECv2 vs TCP: Throughput vs Loss Rate "
...
def plot_throughput_vs_size(rows):
    ...
    x = range(len(labels))
    w = 0.20
    ax.bar([i - 1.5*w for i in x], rudp,        w, label="RUDP",       color="tab:blue")
    ax.bar([i - 0.5*w for i in x], rudp_fec,    w, label="RUDP-FEC",   color="tab:green")
    ax.bar([i + 0.5*w for i in x], rudp_fec_v2, w, label="RUDP-FECv2", color="tab:red")
    ax.bar([i + 1.5*w for i in x], tcp,         w, label="TCP",        color="tab:orange")
    ax.set_xticks(list(x))
    ax.set_xticklabels(labels)
    ax.set_xlabel("File size")
    ax.set_ylabel("Throughput (Mbps)")
    ax.set_title("RUDP vs RUDP-FEC vs RUDP-FECv2 vs TCP at 0% Loss (median of trials)")
    ax.grid(True, axis="y", alpha=0.3)
    ax.legend()
    all_vals = rudp + rudp_fec + rudp_fec_v2 + tcp
    for i, (a, b, c, d) in enumerate(zip(rudp, rudp_fec, rudp_fec_v2, tcp)):
        ax.text(i - 1.5*w, a + max(all_vals) * 0.01, f"{a:.0f}", ha="center", fontsize=8)
        ax.text(i - 0.5*w, b + max(all_vals) * 0.01, f"{b:.0f}", ha="center", fontsize=8)
        ax.text(i + 0.5*w, c + max(all_vals) * 0.01, f"{c:.0f}", ha="center", fontsize=8)
        ax.text(i + 1.5*w, d + max(all_vals) * 0.01, f"{d:.0f}", ha="center", fontsize=8)
