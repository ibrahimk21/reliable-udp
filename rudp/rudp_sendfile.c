...
{
    if (argc < 4) {
        fprintf(stderr, "Usage: %s <server_ip> <server_port> <file_path> [-fec K=N] [-fecv2 K=N]\n", argv[0]);
        return 1;
    }
    ...
    if (use_fec_v2) rudp_sender_set_fec_v2(&sender, fec_k, 1);
    else if (use_fec) rudp_sender_set_fec(&sender, fec_k, 1);
    ...
    if (file_size > 0) {
        if (use_fec_v2)
            ret = rudp_send_block_fec(&sender, file_data, (int)file_size,
